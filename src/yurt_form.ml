...
let urlencoded_json body : Ezjsonm.t Lwt.t =
  urlencoded_list body >|= fun f ->
  `O (List.map (fun (k, v) -> (k, `A (List.map Ezjsonm.encode_string v))) f)
...
let split_semicolon (s : string) : string list = Str.split semicolon_regexp s

let get_attr (m : multipart) (attr : string) : string list =
  try Hashtbl.find m.attr attr with Not_found -> []

(** Return true when the multipart object has a filename attribute *)
let is_file (m : multipart) : bool =
  match get_attr m "filename" with [] -> false | _ -> true

let is_multipart_regexp = Str.regexp "multipart/.*"

let is_multipart req : bool =
  let content_type =
    Yurt_util.unwrap_option_default
      (Header.get req.Request.headers "Content-Type")
      ""
  in
...
let multipart req body : multipart list Lwt.t =
  ...
  let boundary =
    ...
    | _ -> raise Invalid_multipart_form
  in
  ...
  let current =
    ref { data = Lwt_stream.of_string ""; attr = Hashtbl.create 16; name = "" }
  in
  ...
  >|= Lwt_list.iter_s (fun line ->
          let _ =
            ...
            | x when x = boundary || x = boundary_a || x = boundary_b ->
                ...
                if bl > 0 || Hashtbl.length c.attr > 0 then
                  ...
                  current :=
                    {
                      data = Lwt_stream.of_string "";
                      attr = Hashtbl.create 16;
                      name = "";
                    }
            (* End of header *)
            | x when !in_header && x = "" -> in_header := false
            (* Get attributes  *)
            | x when !in_header ->
                let m = "Content-Disposition: form-data; " in
                let mlen = String.length m in
                if String.length x >= String.length m && String.sub x 0 mlen = m
                then
                  let x =
                    String.sub x mlen (String.length x - String.length m)
                  in
                  ...
                  List.iter
                    (fun part ->
                      ...
                      if k == "name" then !current.name <- v
                      else if Hashtbl.mem !current.attr k then
...
let parse_form req body : form Lwt.t =
  if is_multipart req then multipart req body >|= fun f -> Multipart f
  else urlencoded body >|= fun f -> Urlencoded f
